  ...
  return (
    <div className="w-full h-full">
      <div className="space-y-6">
        ...
        {/* Guardian Connection */}
        <div className="border border-gray-200 rounded-lg p-6">
          <h3 className="text-[16px] font-[500] mb-4">GUARDIAN CONNECTION</h3>
          <div className="flex items-center gap-2 mb-2">
            <span className={`w-2 h-2 rounded-full ${
              guardianStatus === 'connected' ? 'bg-green-500' :
              guardianStatus === 'connecting' ? 'bg-yellow-500 animate-pulse' : 'bg-red-500'
            }`} />
            <span className="text-[12px]">{guardianStatus}</span>
          </div>
          <div className="text-[11px] text-gray-500 break-all">{guardianUrl}</div>
        </div>
      </div>
    </div>
  );
